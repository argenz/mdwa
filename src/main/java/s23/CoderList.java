...
public class CoderList extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LoggerFactory.getLogger(CoderList.class);
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        logger.trace("called");
        try (DaoCoder dao = new DaoCoder(ds)) {
            request.setAttribute("coders", dao.getAll());
            request.getRequestDispatcher("/esercizi/welcome.jsp").forward(request, response);
        }
    }
